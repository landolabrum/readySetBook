...
const pwFields: FormField[] = [
  { name: "password", label: "password", type: "password", placeholder: "password", required: true, width: "50%" },
  { name: "confirm_password", label: "confirm password", type: "password", placeholder: "confirm password", required: true, width: "50%" }
];
...
const SignUp = ({ hasPassword = true, btnText, onSuccess, title, fieldConfig }: ISignUp): React.JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (!exists && clearance == 3)
      setFields([
        ...fields,
        { name: "clearance", value: 10, label: "clearance", step: 1, placeholder: "clearance", type: "tel" },
      ]);
  }, [clearance == 3]);
  return (
    <>
