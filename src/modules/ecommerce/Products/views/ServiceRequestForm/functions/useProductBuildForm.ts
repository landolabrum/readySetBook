...
const useProductBuildForm = () => {
    ...
    const setField = ({ name, value, checked, path = '', type, quantity }: { name: string; value: any; checked?: boolean; path?: string | string[], type?: string, quantity?: number }) => {
        let updatedFields: IFormField[];

        // Removal: explicit checkbox uncheck, or quantity decremented to 0
        if ((checked === false && type === 'checkbox') || quantity === 0) {
            ...
            const prevQty = fieldIndex !== -1 ? Number(updatedFields[fieldIndex].quantity) : undefined;
            const nextQty = quantity != null ? quantity : (prevQty && prevQty > 0 ? prevQty : 1);

            if (fieldIndex !== -1) {
                updatedFields[fieldIndex] = { name, value, checked: true, path, quantity: nextQty };
            } else {
                updatedFields.push({ name, value, checked: true, path, quantity: nextQty });
            }
        }
