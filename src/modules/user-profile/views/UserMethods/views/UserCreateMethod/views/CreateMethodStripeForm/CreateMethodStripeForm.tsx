...
const CreateMethodStripeForm = ({ onSuccess, user, shippable, success_url = "/checkout" }: IAccountCreateMethod) => {
    ...
    const paymentElementOptions = {
        ...
        paymentMethodOrder: ['apple_pay', 'google_pay', 'card', 'link', 'ideal', 'bancontact'],
    };
    ...
    const onSubmit = useCallback(async (event: any) => {
        ...
        if (!stripe || !elements) {
            ...
            setNotification({
                active: true,
                list: [{ name: "Stripe unavailable", message: "Payment form is still loading. Please try again." }]
            });
            return;
        }
            ...
            <form onSubmit={onSubmit} className='account-create-method'>
                <div className='account-create-method__content'>
                    ...
                    <div className='account-create-method__submit'>
                        <UiButton type="submit" size='sm' traits={{ afterIcon: "fa-credit-card-front", padding: '10px' }} variant={elements ? 'link' : 'disabled link'}>
                            Verify & Save
                        </UiButton>
                    </div>
                </div>
