  ...
  return (
    <>
      ...
      <div className='lights'>
        ...
        {hueData && !isAll && <AdaptGrid xs={1} sm={3} md={6} gap={15}>
          {Object.entries(hueData).map(([key, light]: any, index: number) =>
            <div
              ...
              {light?.view !== 'color' ? (
                <UiBar
                  color={light?.hex ? `#${reverseString(light.hex.substring(1, light.hex.length))}` : undefined}
                  onChange={(value) => {
                ...
                <ColorPicker
                  value={light.hex}
                  onChange={(e: any) => multiHomeService('color', { id: light.id_, hex: e?.target?.value, name: light.name })}
                />
              )}
            </div>
          )}
