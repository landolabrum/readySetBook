...
const WindowCaptureOverlay: React.FC<WindowCaptureOverlayProps> = ({
  ...
  const getConstraints = useCallback((): DisplayMediaStreamOptions => {
    ...
    // Request system audio capture for window/screen shares (Chrome 105+, requires PipeWire on Linux).
    // Falls back gracefully — Chrome ignores the hint when unsupported.
    if (systemAudio) {
      (opts as any).systemAudio = 'include';
    }
    return opts;
