...
const AdminProducts: React.FC = () => {
  ...
  const { user, others } = useAdminLevel();
  const [view, setView] = useState<'list' | 'add' | 'product'>('list');
  const [product, setProduct] = useState<any>();
  // Track raw input separately so the UI reflects keystrokes instantly
  ...
  // Merchant selection is only allowed for high-clearance admins
  const canSelectMerchant =
    typeof others?.creation === "number" && others.creation > 11;
  ...
  const handleSearch = useCallback((term: string) => {
    ...
    searchTimeout.current = setTimeout(() => setSearchTerm(normalized), 250);
  }, []);
  ...
  useEffect(() => {
    ...
    products.forEach((field: any) => {
      ...
      const existing = byProductId.get(field.id);

      // derive how many prices this entry represents
      const pricesArray = (field as any).prices;
      const additionalCount =
        Array.isArray(pricesArray) && pricesArray.length > 0
          ? pricesArray.length
          : field.price
            ? 1
            : 0;
      ...
      if (!existing) {
        const ctx: any = {
          ...
          price: field.price ? numberToUsd(field.price.unit_amount) : undefined,
          prices: additionalCount,
        ...
        byProductId.set(field.id, ctx);
      } else {
        existing.prices = (existing.prices || 0) + additionalCount;
      }
    });
  ...
  const midOptions = useMemo(() => {
    // Build merchant options from the same row data the table uses,
    // so we reflect the mids actually visible in the grid.
    const base = getUniqueMids(baseRows || []).map((mid) => ({
      label: mid,
      name: `merchant-${mid}`,
      disabled: !canSelectMerchant,
    }));
