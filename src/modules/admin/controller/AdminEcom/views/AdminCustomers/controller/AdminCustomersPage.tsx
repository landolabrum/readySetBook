...
const AdminCustomersPage: React.FC = () => {
  const {
    ...
    changeLimit,
    applySearch,
  } = useAdminCustomers(1, 10);

  const handlePageChange = (newPage: number) => {
    if (!newPage || newPage === page) return;
    const next = newPage < 1 ? 1 : newPage;
    goToPage(next);
  };

  const handleLimitChange = (newLimit: number) => {
    if (!newLimit || newLimit === perPage) return;
    changeLimit(newLimit);
  };

  // Debug: log hook state so we can confirm page/rows changing
  // eslint-disable-next-line no-console
  console.debug("[AdminCustomersPage] state", {
    page,
    perPage,
    total,
