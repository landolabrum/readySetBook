...
console.log("[ DIsPLAY FIELDS ]", displayFields )
  ...
  // Keys that should never be sent to the admin update endpoint
  const NON_UPDATABLE_KEYS = new Set([
    'methods', 'object', 'balance', 'created', 'currency',
    'customer_account', 'default_source', 'delinquent', 'description',
    'discount', 'invoice_prefix', 'invoice_settings', 'livemode',
    'next_invoice_sequence', 'preferred_locales', 'shipping',
  ...
  const getDifferences = (initialObj: any, updatedObj: any) => {
    ...
    const compare = (initial: any, updated: any, path: string[] = []) => {
      for (const key in updated) {
        // Skip non-updatable top-level keys
        if (path.length === 0 && NON_UPDATABLE_KEYS.has(key)) continue;

        const initVal = initial?.[key];
        const updVal = updated[key];

        if (Array.isArray(updVal)) {
          // Deep-compare arrays via JSON to avoid false positives from reference inequality
          if (JSON.stringify(initVal) !== JSON.stringify(updVal)) {
            const fullPath = [...path, key].join('.');
            set(differences, fullPath, updVal);
          }
        } else if (typeof updVal === "object" && updVal !== null) {
          // Recursively compare objects
          compare(initVal || {}, updVal, [...path, key]);
        } else if (updVal !== initVal) {
          const fullPath = [...path, key].join('.');
          set(differences, fullPath, updVal);
        }
      }
    ...
    const modifyCustomerService = async (request: any) => {
      try {
        ...
        if (response?.status === 'updated') {
          setNotification({ active: true, list: [{ label: 'success', message: 'Customer updated' }] });
          // Refresh to pick up the persisted state
          setCustomerState(undefined);
          setInitialCustomer(undefined);
        } else if (response) {
          openModal({ children: JSON.stringify(response, null, 2) });
        }
      } catch (error: any) {
        console.error({ error });
        setNotification({ active: true, dismissable: true, apiError: error });
      }
    };
