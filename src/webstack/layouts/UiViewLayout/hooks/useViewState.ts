...
export const useViewState = (views?: any, initialKey?: string | number) => {
    ...
    // Keep the active view element in sync when the `views` map changes
    // (e.g., same key like "list" but with new props/data).
    useEffect(() => {
        if (!views || history.length === 0) return;
        const lastKey = history[history.length - 1];
        const nextView = views[lastKey];
