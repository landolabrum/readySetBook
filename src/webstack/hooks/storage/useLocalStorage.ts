...
const useLocalStorage = (incomingKey?: string) => {
  ...
  const getLocalItem = useCallback((name: string) => {
    if (!isBrowser()) return null;
    const raw = localStorage.getItem(name);
    return unwrapStored(raw);
  }, []);

  const applyLocalData = (data: StorageData) => {
  ...
  const setLocalItem = useCallback((name: string, val: any, options?: StorageOptions) => {
    ...
    try {
      ...
      applyLocalData(getLocalData()); // update this hook without re-parsing twice
  ...
  const deleteLocalItem = useCallback((name: string) => {
    if (!isBrowser()) return;
    localStorage.removeItem(name);
    applyLocalData(getLocalData());
    notifyLocal(name);
  }, []);
