...
import { DM_UNREAD_KEY } from "~/src/modules/direct/utils/dmStorage";
...
const Navbar = (items:any=[]) => {
  ...
  const [currentRoutes, setCurrentRoutes] = useState<IRoute[]>(items);
    ...
    const brandTitle =
      <>
        <h1 onClick={() => handleMobileClick({ href: '/' })}>
          {merchantName && <UiIcon icon={`${keyStringConverter(environment.merchant?.display || environment.merchant.name, { dashed: true }) }-logo`} />}
  ...
  const merchantName = keyStringConverter(environment.merchant?.display || environment.merchant.name, { dashed: false });
  const isBrandRoute = (route: IRoute) => ([environment.merchant.name, environment.merchant?.display])?.includes(route.label);
  ...
  const modals: any = {
    login: { children: <Authentication view="sign-up" /> },
  ...
  useEffect(() => {
    syncUnread();
    const id = window.setInterval(syncUnread, 60_000);
    return () => window.clearInterval(id);
  }, [syncUnread]);

  useEffect(() => {
    syncUnread();
    const handleFocus = () => syncUnread();
    const handleStorage = () => syncUnread();
  ...
  return (
    <>
      ...
      <nav id="nav-bar" className={navClass}>
        {merchantName &&
          <div className="navbar__brand" onClick={() => handleSelect({ href: '/' })}>
            <div className="navbar__brand_logo">
              <UiIcon icon={`${environment.merchant.name}-logo`} />
            </div>
            <div className="navbar__brand_name">
              {environment?.merchant?.display || merchantName}
            </div>
          </div>
        }
        <div className="navbar" ref={navRef}>
          ...
          <div ref={navItemsRef} className={`nav-bar__nav-items`}>
            {currentRoutes?.length && currentRoutes.map((route, key) => {
              return <div
                key={key}
              ...
              >
                {route.href !== '/cart' ? !route?.items ? (
                  <UiButton
                    traits={route?.icon ? (
                      isBrandRoute(route) ? {
                        beforeIcon: { icon: route.icon, badge: route.badge }
                      } : {
                        afterIcon: { icon: route.icon, badge: route.badge }
                      }
                    ) : undefined}
                    variant="nav-item"
