            ...
            itemsRef.current.forEach((item: any, index: number) => {
                if (item) {
                    const angle = items.length === 1
                        ? -(Math.PI / 4)
                        : (Math.PI / 2) * (index / (items.length - 1)) - (Math.PI / 2);
        ...
        <>
            ...
            <div className='menu-fan'>
                <div
                    ...
                    style={{
                        padding: `${distance * 0.1}px`,
                        bottom: `${distance * 0.15}px`,
                        right: `${distance * 0.15}px`,
                    }}
                    onClick={handleClick}
                >
                    <UiIcon icon={icon}
                    color={show?"var(--primary-30)":"var(--gray-80)"} size={distance * 0.2} />
                </div>
                <div
