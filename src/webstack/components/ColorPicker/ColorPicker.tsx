...
import UiInput from '../UiForm/components/UiInput/controller/UiInput';
import { IInput } from '@webstack/models/input';

const ColorPicker = ({ value, onChange, name }: IInput) => {
  ...
  const debouncedOnChange = useCallback(
    debounce((pickedColor: string) => {
      if (onChange) {
        onChange({ target: { name, value: pickedColor } } as any);
      }
    }, 1000),
    [onChange, name]
  );
...
const handleInput = (e: any)=>{
  ...
  setColor(`#${value}`);
  if (value.length === 6) debouncedOnChange(`#${value}`);
};

useEffect(() => {
  if(!color){
    setColor(value?value:"#fff");
  }
}, [value, setColor, handleInput]);
