...
const AdaptTableCell: NextComponentType<NextPageContext, {}, Props> = ({ cell, data }: Props) => {
  ...
  if (valid)
    return (
      <>
        <style jsx>{styles}</style>
        {cell === "icon-label" && (<div className='adaptable-cell__icon-label'>
        <UiIcon height="" icon={typeof data?.icon=="string"&&data.icon||data?.icon}/>
        ...
        {cell === "member" && (
          <div className="adaptable-cell__member-container">
            ...
            <div className="adaptable-cell__member">
              ...
              {data.email && <div className="adaptable-cell__member-email">
                <UiButton variant="link" traits={{
                  afterIcon: {
                    icon: "fa-copy",
              ...
              {data.phone && <div>
                <UiIcon icon="fa-phone" size={12} color="#999" />
