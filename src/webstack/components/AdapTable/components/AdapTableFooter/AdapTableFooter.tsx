...
export default function AdapTableFooter({
  ...
  const selectOptions: any[] = ITEMS_PER_PAGE_OPTIONS
    .filter((o) => !total || o <= total)
    .map((o) => ({ label: String(o), value: String(o) }));

  const pageRange = Array.from({ length: page + 2 - page + 1 }, (_, i) => page + i);

  const handleLimit = (selected: any) => {
    const raw = typeof selected === "object" ? selected?.value : selected;
    const newPageLimit = Number(raw);

    if (!Number.isFinite(newPageLimit) || newPageLimit <= 0) {
      return;
    }

    // For external pagination we always reset to page 1 when limit changes.
    handlePageChange(1);
