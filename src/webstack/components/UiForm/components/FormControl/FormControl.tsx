...
export type ITraits = {
  beforeIcon?: IFormControlIcon;
  afterIcon?: IFormControlIcon;
  width?: number | string;
  ...
  return (
    <>
      ...
      <div className={propClasses('form-control')} ref={ref}>
        {label && (
          <div className={propClasses("form - control__header")}>
            <label htmlFor={name}>
              {formControlLabel}
        ...
        <div className={propClasses('form-control__element')} onClick={onClick}>
...
function renderIcon(iconProps: IFormControlIcon | undefined, position: string, size?: string, variant?: IFormControlVariant, alt?: string, badge?: string) {
  ...
  return (
    <>
      ...
      <div className={`${iCls} ${iCls}__${position} ${variant ? ` ${iCls}-${variant}` : ""} ${size ? ` ${iCls}-${size}` : ""} ${onClick ? ' icon-click' : ''}`}>
        <UiIcon icon={icon} onClick={onClick} color={color} alt={alt} badge={badge} />
      </div>
    </>
  );
}
