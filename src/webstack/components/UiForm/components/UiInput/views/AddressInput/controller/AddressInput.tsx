...
import styles from '@webstack/components/UiForm/components/UiInput/controller/UiInput.scss';
...
const AutocompleteAddressInput = ({
  ...
  const reverseGeocodedRef = useRef<Set<string>>(new Set());
  ...
  /* ---------------- coordinate helpers ---------------- */
  const formatCoordinates = useCallback((lat?: number | null, lng?: number | null): string => {
  ...
  const hasHumanReadableAddress = useCallback((addr: any): boolean => {
  ...
  useEffect(() => {
    ...
    // If we have human-readable address, show it
    if (hasHumanReadableAddress(address)) {
      const s = `${address?.line1 ? address.line1 + ', ' : ''}${address?.line2 ? address.line2 + ' ' : ''}${address?.city ? address.city + ' ' : ''}${address?.state ? address.state + ', ' : ''}${address?.postal_code ? address.postal_code + ', ' : ''}${address?.country ? address.country : ''}`;
      setDisplay(s.trim());
    } else if (address.lat != null && address.lng != null) {
      // Otherwise show coordinates if we have them
      setDisplay(formatCoordinates(address.lat, address.lng));
    } else {
      setDisplay('');
    }
  }, [address, hasHumanReadableAddress, formatCoordinates]);
  ...
  const reverseGeocode = useCallback((lat: number, lng: number) => {
    ...
    if (geocodeCacheRef.current.has(cacheKey)) {
      ...
      setAddressAndDisplay(cached);
      return;
    }

    // Check if we already geocoded this coordinate
    if (reverseGeocodedRef.current.has(cacheKey)) return;

    reverseGeocodedRef.current.add(cacheKey);
    setIsGeocoding(true);
    ...
    geocoder.geocode(
      ...
      (results: any[], status: any) => {
        ...
        if (status !== 'OK' || !results?.length) {
          // Fallback: create minimal address with coordinates
          const fallback = {
            line1: '',
            line2: '',
            city: '',
        ...
        geocodeCacheRef.current.set(cacheKey, addrObj);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (coords) {
      // Valid coordinates typed - reverse geocode them
      debounce(() => reverseGeocode(coords.lat, coords.lng));
    } else {
  ...
  const fullTraits: ITraits | undefined = useMemo(() => ({
    ...traits,
    afterIcon: { icon: (locating || isGeocoding) ? 'spinner' : 'fa-crosshairs', onClick: handleCrosshairsClick },
    variant: 'address',
  }), [traits, locating, isGeocoding, handleCrosshairsClick]);
