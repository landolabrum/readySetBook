...
const RADIAL_RADIUS_RATIO = 0.72; // shrink the radius so outer node labels stay inside the square
...
const ThreeTree: React.FC<IThreeTree> = ({
  ...
  const IS_RADIAL = variant?.includes('radial') || false  ;
  ...
  const update = (source: any, i: number) => {
    ...
    const treeLayout = IS_RADIAL
      ? d3.tree().size([2 * Math.PI, (Math.min(container.offsetWidth, container.offsetHeight) / 2) * RADIAL_RADIUS_RATIO])
    ...
    const svg = d3.select(svgRef.current as SVGSVGElement)
      .attr('class', 'd3-svg')
      .attr('width', '100%')
      .attr('height', contentHeight || '600px')
    ...
    if (IS_RADIAL) {
      ...
      // Keep the radial diagram square and bounded by the viewport so it sits
      // cleanly inside the parent instead of growing unbounded vertically.
      const maxByViewport = height ? height * 0.82 : container.offsetWidth;
      const radialSize = Math.round(Math.min(container.offsetWidth, maxByViewport));
      if (Math.abs(contentHeight - radialSize) > 1) setContentHeight(radialSize);
    } else {
      // Spread columns across the full container width so the tree fills the
      // horizontal space; reserve a right margin for the deepest leaf labels.
      const maxDepth = d3.max(nodes, (d: any) => d.depth) || 0;
      const leftOffset = TITLE_SIZE;
      const rightPadding = container.offsetWidth * 0.15;
      const usableWidth = Math.max(container.offsetWidth - leftOffset - rightPadding, 0);
      nodes.forEach((d: any) => {
        d.y = maxDepth ? leftOffset + (d.depth / maxDepth) * usableWidth : leftOffset;
      });
    }
...
useEffect(() => {
  ...
  // variant,
  treeData, width, height, selectedData, transform, contentHeight]);
