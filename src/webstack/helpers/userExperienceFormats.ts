 ...
 * Examples: 905000 -> "15m", 90061000 -> "1d 1h" (largest=2)
 */
export function durationFormat(ms: number, opts: { largest?: number; style?: 'short' | 'long' } = {}) {
  const style = opts.style ?? 'short';
  const largest = Math.max(1, opts.largest ?? 2);
  const neg = ms < 0;
  let remain = Math.abs(ms);
  ...
  const parts: string[] = [];
  const push = (val: number, short: string, long: string) => {
    if (val > 0 && parts.length < largest) parts.push(style === 'short' ? `${val}${short}` : `${val} ${long}${val !== 1 ? 's' : ''}`);
  };

  push(d, 'd', 'day');
  push(h, 'h', 'hour');
  push(m, 'm', 'minute');
  if (parts.length === 0) push(sec, 's', 'second');

  const head = parts.join(style === 'short' ? ' ' : ', ');
  return neg ? `-${head}` : head || (style === 'short' ? '0s' : '0 seconds');
}

/**
 * Human label for time until a target date/time. Past times are labeled "x ago".
 */
export function timeUntil(target: dateProps, options?: { largest?: number; style?: 'short' | 'long' }) {
  const toDate = (d: any) => (typeof d === 'string' || typeof d === 'number' ? new Date(d) : d);
  const t = toDate(target);
  if (!(t instanceof Date) || isNaN(t.getTime())) return 'n/a';
  const delta = t.getTime() - Date.now();
  const label = durationFormat(Math.abs(delta), { largest: options?.largest ?? 2, style: options?.style ?? 'short' });
  return delta >= 0 ? label : `${label} ago`;
}
