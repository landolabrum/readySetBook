...
export default function keyStringConverter(str: string, options?: IKeyStringConverterOptions): string {
  ...
  if (result?.length && transform) {
    ...
    else if (transform === 'uppercase') result = result.toUpperCase();
    else if (transform === 'capitalize') result = result.replace(/\b\w/g, char => char.toUpperCase());
    else console.error(`[ KeyStringConverter ]: textTransform defined incorrectly as '${transform}'`);
  }
