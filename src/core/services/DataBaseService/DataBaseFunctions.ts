...
export function normalizeDelete(req: DeleteRequest | IDataBaseData) {
  if (isDelete(req)) {
    ...
    // Wrap simple where map into { exact: ... } so the backend's WherePayload validates.
    const w = d.where ?? undefined;
    const wrapped = w && typeof w === 'object' && !('exact' in (w as any)) ? { exact: w } : w;
    return {
      tableName: d.tableName,
      where: wrapped as any,
    };
  }
  return serializeLegacyEnvelope(req as IDataBaseData);
}
