...
/* --------------------- API paths --------------------- */
const PATH_SELECT = "db/select";
const PATH_INSERT = "db/insert";
const PATH_UPDATE = "db/update";
const PATH_DELETE = "db/delete";
const PATH_CREATE_TABLE = "db/create_table";
const PATH_DROP_TABLE = "db/drop_table";
const PATH_LIST_TABLES = "db/list_tables";
const PATH_DESCRIBE_TABLE = "db/describe_table";
const PATH_INSERT_EVENT = "db/insert_event";
const PATH_OVERLAY_PING = "db/overlay_ping";          // NEW
const PATH_OVERLAY_STREAM = "db/overlay_stream";      // NEW
/* const PATH_UPSERT = "db/upsert";  // uncomment if you add a backend endpoint */
/* ===================================================== */
const PATH_STREAM_CFG_GET = "db/stream_config";
const PATH_STREAM_CFG_SET = "db/stream_config_set";
const PATH_STREAM_CFG_REVOKE = "db/stream_config_revoke";
const PATH_STREAM_CREATE = "db/stream_create";
const PATH_STREAM_UPDATE = "db/stream_update";
const PATH_STREAM_DELETE = "db/stream_delete";
const PATH_STREAM_LIST = "db/stream_list";
const PATH_STREAM_START = "db/stream_start";
const PATH_STREAM_END = "db/stream_end";
...
export default class DataBaseService extends ApiService implements IDataBaseService {
  ...
  public async deleteData(req: DeleteRequest | IDataBaseData): Promise<ODataBaseData> {
    ...
    // Prefer POST (body-friendly); fall back to DELETE if server only allows DELETE.
    try {
      return await this.post<typeof payload, ODataBaseData>(PATH_DELETE, payload);
    } catch (err: any) {
      if (err?.status === 405) {
        // Some proxies/instances still expose only DELETE
        return this.delete<typeof payload, ODataBaseData>(PATH_DELETE, payload);
      }
      throw err;
    }
  }
