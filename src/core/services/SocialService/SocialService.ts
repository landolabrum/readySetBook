...
export default class SocialService extends ApiService implements ISocialService {
  ...
  public async instagramDebug(opts?: { auth_proxy?: boolean }): Promise<IGDebug> {
    const path = opts?.auth_proxy ? "instagram/debug_fingerprint?auth_proxy=1" : "instagram/debug_fingerprint";
    return this.get(path);
  }

  public async instagramLiveCreate(payload: IGLiveCreateRequest): Promise<IGLiveCreateResponse> {
    if (!payload?.title) throw new Error("title is required");
    return this.post("instagram/live/create", payload);
  }

  public async instagramKeepAlive(username: string): Promise<IGKeepAliveResponse> {
    if (!username) throw new Error("username is required");
    return this.get(`instagram/keepalive?username=${encodeURIComponent(username)}`);
  }
