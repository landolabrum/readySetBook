...
export type UseInstagram = {
  ...
  lastDebug?: IGDebug;
  lastLiveCreate?: IGLiveCreateResponse;
  lastKeepAlive?: IGKeepAliveResponse;
  ...
  liveCreate(payload: IGLiveCreateRequest): Promise<IGLiveCreateResponse>;
  keepAlive(username: string): Promise<IGKeepAliveResponse>;
...
const useInstagram = (): UseInstagram => {
  ...
  const [lastLiveCreate, setLastLiveCreate] = React.useState<IGLiveCreateResponse>();
  const [lastKeepAlive, setLastKeepAlive] = React.useState<IGKeepAliveResponse>();
  ...
  const liveCreate = React.useCallback(
    async (payload: IGLiveCreateRequest) => {
      const res = await run<IGLiveCreateResponse>("live_create", () => socialService.instagramLiveCreate(payload));
      setLastLiveCreate(res);
      return res;
    },
    [run, socialService]
  );
  ...
  const keepAlive = React.useCallback(
    async (username: string) => {
      const res = await run<IGKeepAliveResponse>("keepalive", () => socialService.instagramKeepAlive(username));
      setLastKeepAlive(res);
      return res;
    },
    [run, socialService]
  );
  ...
  const resetAll = React.useCallback(() => {
    ...
    setLastLiveCreate(undefined);
    setLastKeepAlive(undefined);
    abort();
  }, [abort]);
